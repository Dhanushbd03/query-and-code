...
interface VectorizationLogsProps {
  onStop: () => Promise<void>;
}

export default function VectorizationLogs({ onStop }: VectorizationLogsProps) {
  const [logs, setLogs] = useState<LogMessage[]>([]);
  const [isConnected, setIsConnected] = useState(false);
  const [connectionError, setConnectionError] = useState<string | null>(null);
  const socketRef = useRef<Socket | null>(null);
  const scrollRef = useRef<HTMLDivElement>(null);
  const { is_vectorizing: isVectorizing } = useVectorization();
    ...
    <div className="h-full flex flex-col bg-ctp-surface1 rounded-lg">
      <div className="flex items-center justify-between p-4 border-b border-ctp-surface2">
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-semibold text-ctp-text">Vectorization Logs</h3>
        ...
        <div className="flex items-center gap-2">
          ...
          {isVectorizing && <Loader2 className="h-4 w-4 animate-spin text-ctp-text" />}
          <Button
            variant="ghost"
            size="icon"
            onClick={clearLogs}
          ...
          <div className="p-4 space-y-2 pb-8">
            ...
            {logs.length === 0 && (
              <div className="flex h-full items-center justify-center text-ctp-subtext0">
                No logs yet. Start vectorization to see logs here.
              </div>
            )}
          </div>
        </ScrollArea>
